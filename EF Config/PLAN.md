...
- **Confirmed working end-to-end on GitHub**: pushed tag `v0.1.0`, both `test-wpf` and `publish-and-release` went green, and `gh release view v0.1.0` confirmed a real release with `EF-Config-Wpf-v0.1.0-win-x64.zip` attached at `https://github.com/mpahee/ef-config/releases/tag/v0.1.0`.
- **Real-world gap found after downloading and running the v0.1.0 release**: the app appeared to do nothing on launch. Root cause turned out to be two separate, stacked issues a first-time downloader would hit in sequence:
  1. **Windows SmartScreen** blocked the unsigned `.exe` outright ("Windows protected your PC") — expected for any unsigned downloaded executable, not a bug, but easy to mistake for "the app is broken" if undocumented.
  2. Even past SmartScreen, the shipped `appsettings.json` only has placeholder Postgres credentials (`Username=CHANGE_ME`) — without a real `appsettings.Local.json` next to the exe, `App.xaml.cs`'s `OnStartup` would throw an *unhandled* exception trying to connect, which WPF surfaces as a generic crash with no useful information.
  - Fix 1 (in-app): wrapped `OnStartup`'s body in try/catch; on failure, show a `MessageBox` explaining the likely cause (SmartScreen bypass already happened by definition if the app got this far; missing/placeholder connection string is the next likely culprit) and the underlying exception message, then `Shutdown(-1)` cleanly instead of an unhandled-exception crash.
  - Fix 2 (release notes): `cd-wpf.yml` now writes a `release-notes.md` (PowerShell here-string) covering both the SmartScreen bypass steps and how to create an `appsettings.Local.json` with a real connection string, passed to `gh release create` via `--notes-file` alongside `--generate-notes` (both combine — generated commit-log notes get appended after the custom notes).
